...
- ⚡ **Sub-Millisecond Deterministic Latency**: Instant regex, AST parsing, and Unicode normalization (<1ms) for prompts, tool calls, and model inputs.
...
- **`INSPECT_SAFETY` Action**: Explicitly audits code snippets, transaction calldata, or dynamic instructions before critical tool execution. Delivers strictly typed `Content` and `ContentValue` callbacks.
...
const result = await inspectSafetyAction.handler(
  ...
  async (callbackContent) => {
    ...
    const { verdict, riskScore, threats, executionTimeMs, oracle } = callbackContent.data as {
...
Comprehensive unit tests cover prompt injection blocking, zero-width character evasion normalization, AST command detection, safe pass-through, and local fail-safe handling.

---

## 📚 Documentation & Specifications

- [📖 ElizaOS Autonomous Agent Security Tutorial](../../docs/ELIZAOS_GUARD_TUTORIAL.md)
- [🛡️ Threat Defense Matrix & Security Taxonomy](../../docs/SECURITY_DEFENSE_MATRIX.md)
