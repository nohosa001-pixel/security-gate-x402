...
gcloud run deploy "$SERVICE_NAME" \
  ...
  --cpu 1 \
  --min-instances 0 \
  --set-env-vars "ENV=$SERVER_ENV,SERVER_WALLET_ADDRESS=$SERVER_WALLET,GATE_PRIVATE_KEY=${GATE_PRIVATE_KEY:-$DEPLOYER_PRIVATE_KEY},NETWORK=polygon,POLYGON_CHAIN_ID=${POLYGON_CHAIN_ID:-137},USDC_CONTRACT_ADDRESS=${USDC_CONTRACT_ADDRESS:-0x3c499c542cEF5E3811e1192ce70d8cC03d5c3359},FACILITATOR_URL=${FACILITATOR_URL:-https://facilitator.x402.org/v2/verify},FREE_TRIAL_LIMIT=3,RATE_LIMIT_PER_MINUTE=120" \
  --project "$PROJECT_ID"
